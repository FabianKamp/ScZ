...
def create_mean_edge_df():
    ...
    FileName = M.createFileName(suffix='Subject-Mean_Edge-Weights',filetype='.pkl')
...
def calc_nbs():
    ...
    for FreqBand in config.FrequencyBands.keys():
        ...
        FileName = M.createFileName(suffix='NBS-pval',filetype='.npy', Freq=FreqBand)
        FilePath = M.createFilePath(M.GroupStatsFC, 'NetBasedStats', FileName)
        np.save(FilePath, pval)
        FileName = M.createFileName(suffix='Component-Adj',filetype='.npy', Freq=FreqBand)
        FilePath = M.createFilePath(M.GroupStatsFC, 'NetBasedStats', FileName)
